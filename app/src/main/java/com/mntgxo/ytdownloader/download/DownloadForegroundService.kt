...
class DownloadForegroundService : Service() {
    ...
    companion object {
        ...
        const val EXTRA_RESULT_MIME = "result_mime"
        ...
        scope.launch {
            ...
            when (result) {
                is FileDownloader.Result.Success -> {
                    ...
                    broadcastComplete(result.filePath, result.mimeType)
                }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            notificationManager.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Downloads", NotificationManager.IMPORTANCE_LOW)
                    .apply { description = "Shows progress for active YouTube downloads" }
            )
        }
    }
    ...
    private fun buildNotification(title: String, progress: Int): Notification =
        NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Downloading").setContentText(title)
    ...
    private fun buildCompleteNotification(title: String): Notification =
        NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setSmallIcon(R.drawable.ic_launcher_foreground)
            .setOngoing(false).setAutoCancel(true).setContentIntent(contentIntent()).build()

    private fun buildFailedNotification(title: String, error: String): Notification =
        NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Download failed").setContentText("$title — $error")
            .setSmallIcon(R.drawable.ic_launcher_foreground)
    ...
    private fun broadcastProgress(pct: Int, speed: Double) =
        sendBroadcast(Intent(ACTION_PROGRESS).apply {
            putExtra(EXTRA_PROGRESS_PCT, pct); putExtra(EXTRA_SPEED, speed); setPackage(packageName)
        })

    private fun broadcastComplete(path: String, mimeType: String) =
        sendBroadcast(Intent(ACTION_COMPLETE).apply {
            putExtra(EXTRA_RESULT_PATH, path); putExtra(EXTRA_RESULT_MIME, mimeType); setPackage(packageName)
        })

    private fun broadcastFailed(error: String) =
        sendBroadcast(Intent(ACTION_FAILED).apply {
            putExtra(EXTRA_ERROR, error); setPackage(packageName)
        })
}
